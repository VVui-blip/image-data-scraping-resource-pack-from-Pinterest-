...
# Endpoint tìm kiếm HTML của DuckDuckGo (không cần JS render, không cần API key)
...
class PinGrabber:
    ...
    # ------------------------------------------------------------------ #
    def find_boards(self, keyword: str, max_results: int = 5) -> List[str]:
        """
        Tìm các URL board Pinterest liên quan đến từ khóa, bằng cách query
        công cụ tìm kiếm DuckDuckGo (bản HTML, không cần JS) với cú pháp
        ...
        công cụ tìm kiếm để tìm ra các board công khai liên quan.
        ...
        try:
            ...
            raise PinGrabberError(f"Không thể tìm kiếm từ khóa: {keyword} ({exc})") from exc
        ...
        for link in soup.find_all("a", href=True):
            ...
            if len(board_urls) >= max_results:
                break

        return board_urls

    def search(self, keyword: str, max_boards: int = 3, max_images_per_board: int = 25) -> List[str]:
        """
        ...
        board_urls = self.find_boards(keyword, max_results=max_boards)

        if not board_urls:
            logger.warning("Không tìm thấy board nào liên quan đến: %s", keyword)
            return []
...
def search(keyword: str, max_boards: int = 3, max_images_per_board: int = 25, timeout: int = 15) -> List[str]:
    """
    ...
    """
    grabber = PinGrabber(timeout=timeout)
    return grabber.search(keyword, max_boards=max_boards, max_images_per_board=max_images_per_board)
